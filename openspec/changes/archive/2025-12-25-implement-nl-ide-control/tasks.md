...
- [x] 实现混合定位方法（GLM + OCR）
  ...
  - 扩大搜索区域后使用 OCR 精确定位
  - 全图 OCR 作为最后手段
- [x] 添加坐标校准器（CoordinateCalibrator）
- [x] 实现 JSON 格式自动修复（处理未转义引号）
- [x] 优化参数提取（支持通用关键字，不限于文件名）
...
### 5.1 用户文档
- [x] 编写安装指南 (`docs/INSTALL.md`)
- [x] 编写使用教程 (`docs/USAGE.md`)
- [x] 编写命令参考手册 (`docs/COMMANDS.md`)
- [x] 编写故障排除指南 (`docs/TROUBLESHOOTING.md`)
...
- [x] 编写 API 参考文档 (`docs/API.md`)
- [x] 编写扩展指南（如何添加新操作）(`docs/EXTENDING.md`)
- [x] 编写贡献指南（包含在 EXTENDING.md 中）
...
2. **M2 - 核心模块完成** ✅：任务 2.1, 2.2, 2.3, 2.4 完成（含混合定位优化）
3. **M3 - 系统集成完成** ✅：任务 3.1, 3.2, 3.3 完成
4. **M4 - 测试完成** ✅：任务 4.1, 4.2, 4.3 完成
5. **M5 - 发布就绪** ✅：任务 5.1, 5.2 完成
